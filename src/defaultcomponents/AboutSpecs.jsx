...
export default function AboutSpecs({ variants }) {
    const brands = [
        ...
        <section className="mb-32 grid lg:grid-cols-2 gap-20 items-center">
            <motion.div variants={variants} className="space-y-8">
                ...
                <div className="space-y-6 text-gray-400 font-light leading-relaxed">
                    <p>
                        For us, a silhouette is more than just rubber and leather—it's a piece of kinetic art.
                        We exist to bridge the gap between design heritage and the desire of the modern collector.
                        Whether it's a court classic or a high-fashion fusion, we treat every pair with the respect it deserves.
                    </p>
                </div>
            ...
            >
                {brands.map((brand, i) => (
                    <div
                        key={i}
                        className="group relative aspect-square bg-white/[0.02] border border-white/5 backdrop-blur-sm flex items-center justify-center hover:border-cyan-500/30 transition-all duration-500 overflow-hidden"
                    >
                        {/* Minimal Corner Accents */}
                        <div className="absolute top-0 left-0 w-1.5 h-1.5 border-t border-l border-white/10 group-hover:border-cyan-500/50 transition-colors" />
                        <div className="absolute bottom-0 right-0 w-1.5 h-1.5 border-b border-r border-white/10 group-hover:border-cyan-500/50 transition-colors" />

                        <brand.icon
                            className={`h-10 w-10 md:h-12 md:w-12 text-gray-600 group-hover:text-cyan-500 transition-all duration-700 ${brand.name === "Converse" ? "scale-[1.8]" : ""}`}
                        />
